...
export default function BlowfishBreathing({
  ...
  useEffect(() => {
    if (enableAudio) {
      if (inhaleAudioRef.current) {
        inhaleAudioRef.current.volume = audioVolume;
      }
      if (exhaleAudioRef.current) {
        exhaleAudioRef.current.volume = audioVolume;
      }
      if (bubbleAudioRef.current) {
        bubbleAudioRef.current.volume = audioVolume;
      }
    }
  }, [enableAudio, audioVolume]);

  // Play bubble sound twice with 2-second gap
  const playBubbleSound = () => {
    if (enableAudio && bubbleAudioRef.current && isActive) {
      // Play first bubble
      bubbleAudioRef.current.currentTime = 0;
      bubbleAudioRef.current.play().catch((error) => {
        console.log('Audio play failed (browser policy):', error.message);
      });
      
      // Play second bubble after 2 seconds
      setTimeout(() => {
        if (bubbleAudioRef.current && isActive) {
          bubbleAudioRef.current.currentTime = 0;
          bubbleAudioRef.current.play().catch((error) => {
            console.log('Audio play failed (browser policy):', error.message);
          });
  ...
  useEffect(() => {
    ...
    const runBreathingCycle = () => {
      switch (breathingPhase) {
        case 'inhale':
          setScale(1.4);
          // Play inhale sound
          if (enableAudio && inhaleAudioRef.current && isActive) {
            inhaleAudioRef.current.currentTime = 0;
            inhaleAudioRef.current.play().catch((error) => {
              console.log('Audio play failed (browser policy):', error.message);
            });
          }
          timeout = setTimeout(() => {
        ...
        case 'exhale':
          setScale(1);
          // Play exhale sound and bubble sound
          if (enableAudio && exhaleAudioRef.current && isActive) {
            exhaleAudioRef.current.currentTime = 0;
            exhaleAudioRef.current.play().catch((error) => {
              console.log('Audio play failed (browser policy):', error.message);
            });
          }
          // Play bubble sound sequence
          playBubbleSound();
          ...
          timeout = setTimeout(() => {
            if (durations.holdEmpty && durations.holdEmpty > 0) {
              ...
              setCycleCount(prev => {
                ...
                // Show badge after 5 completed cycles
    ...
    return () => {
      ...
      // Clean up audio when component unmounts or effect reruns
      if (!isActive) {
        if (inhaleAudioRef.current) {
    ...
    <div className="flex flex-col items-center justify-center h-full bg-gradient-to-br from-blue-50 to-cyan-50 p-8 rounded-xl">
      ...
      {showBadge && (
        <GoodJobBadge 
          ...
          message="Amazing! You completed 5 breathing cycles! 🐡"
          stickerType="pufferfish"
        />
      )}

      {/* Audio elements for breathing sounds */}
      {enableAudio && (
        <>
          <audio ref={inhaleAudioRef} preload="auto">
            <source src="/breath-in-242641.mp3" type="audio/mpeg" />
            Your browser does not support the audio element.
          </audio>
          <audio ref={exhaleAudioRef} preload="auto">
            <source src="/breath-out-242642.mp3" type="audio/mpeg" />
            Your browser does not support the audio element.
          </audio>
          <audio ref={bubbleAudioRef} preload="auto">
